...
// xorshift64 PRNG
// ---------------------------------------------------------------------------
static uint64_t xorshift64_state = UINT64_C(0x256256256256256B);
...
// ---------------------------------------------------------------------------
int main(void) {
    printf("=== Layer 3 (Step 9-3A): bavc_commit_faest_256 CPU vs GPU test ===\n");
    printf("PRNG seed: 0x256256256256256B\n\n");

    const faest_paramset_t* params = faest_get_paramset(FAEST_256S);
    if (!params) {
        fprintf(stderr, "faest_get_paramset(FAEST_256S) returned NULL\n");
        return 1;
    }
    ...
    for (int trial = 0; trial < 3; ++trial) {
        ...
        /* GPU path */
        bavc_t gpu_bavc = {0};
        bool ok = bavc_commit_faest_256_cuda_test(&gpu_bavc, root_key, iv, params);
        if (!ok) {
            printf("  [FAIL] bavc_commit_faest_256_cuda_test returned false\n");
            bavc_clear(&cpu_bavc);
            bavc_clear(&gpu_bavc);
            ++total_fail;
            continue;
