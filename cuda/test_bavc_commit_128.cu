...
// ---------------------------------------------------------------------------
int main(void) {
    printf("=== Layer 3 (Step 3A): bavc_commit_faest_128 CPU vs GPU test ===\n");
    printf("PRNG seed: 0x123456789abcdef0\n\n");

    const faest_paramset_t* params = faest_get_paramset(FAEST_128S);
    if (!params) {
        fprintf(stderr, "faest_get_paramset(FAEST_128S) returned NULL\n");
        return 1;
    }
    ...
    for (int trial = 0; trial < 3; ++trial) {
        ...
        /* GPU path */
        bavc_t gpu_bavc = {0};
        bool ok = bavc_commit_faest_128_cuda_test(&gpu_bavc, root_key, iv, params);
        if (!ok) {
            printf("  [FAIL] bavc_commit_faest_128_cuda_test returned false\n");
            bavc_clear(&cpu_bavc);
            bavc_clear(&gpu_bavc);
            ++total_fail;
            continue;
